...
RC Table::open(const char *meta_file, const char *base_dir, CLogManager *clog_manager)
{
  ...
  for (int i = 0; i < index_num; i++) {
    ...
    for (int j = 0; j < index_fields->size(); j++) {
      const char *field_name = index_fields->at(j).data();
...
RC Table::create_index(Trx *trx, const char *index_name, bool unique, int attribute_num, const AttrInfo *attribute_name)
{
  ...
  for (int i = 0; i < attribute_num; i++) {
    if (common::is_blank(attribute_name[i].name)) {
      LOG_INFO("Invalid input arguments, table name is %s, attribute name is blank", name());
      return RC::INVALID_ARGUMENT;
    }
    field_names.push_back(attribute_name[i].name);
  }
  if (table_meta_.find_index_by_field(field_names) != nullptr) {
    std::string exist_index_attribute = "";
    for (int i = 0; i < attribute_num - 1; i++) {
      exist_index_attribute.append(attribute_name[i].name);
      exist_index_attribute.append(",");
    }
    exist_index_attribute.append(attribute_name[attribute_num].name);
    LOG_INFO("Invalid input arguments, table name is %s, attribute {%s} exist index",
        name(),
        index_name,
        exist_index_attribute.c_str());
    return RC::SCHEMA_INDEX_EXIST;
  }
